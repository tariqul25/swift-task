...
const AddNewTask = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Step 2: Update Coins if task is inserted
      if (taskRes.data.insertedId) {
        const updatedCoinAmount = coins - totalPayableAmount;

        const coinRes = await axios.patch(
        ...
        if (coinRes.status === 200) {
          Swal.fire({
            icon: 'success',
            title: 'Task Created!',
            text: 'Task created and coins updated successfully!'
          });
        }
      }
    } catch (err) {
